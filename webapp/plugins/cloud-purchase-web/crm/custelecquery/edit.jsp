...
<mk-top-bottom height="100%" top_height="auto">
	<mk-search-panel title="客户售电合同电量分析" slot="top" deployable="false">
        ...
        <su-form-group :label-name="formFields.year.label" class="form-control-row "  col="4" 
            ...
            <su-datepicker :max.sync="maxYear" :min.sync="minYear" format="YYYY"  :data.sync="params.scElectricityInfoDeatil.year" ></su-datepicker>
        </su-form-group>
    </mk-search-panel>
...
// 					url: "${Config.getConfig('selling.bigdata.url')}/powerFore/LongAssAction!predictLongAssDl.action?year=2018&consNo=4ddc5e74661f47ee89ecd40925bc0b97&av=b7b7596fffc91a2bc016c1128df857fd",
					...
					success:function(data){
						if(data != '' && data != null){
							...
							//dataForecastPqList预测数据为13个  绘图是去掉最后的总和
		...
		initYear:function(){
			...
			me.year = year;
			me.minYear = year+"-01-01";
			me.maxYear = (parseInt(year)+1)+"-12-30";
		},
...
// 			if((ym+1) == this.params.scElectricityInfoDeatil.ym){	//预估年为当前年 + 1
// 				index = 4;
// 				list = [firstData,secondData,thirdData,lastData];
// 			}else if(ym == this.params.scElectricityInfoDeatil.ym){	//预估年为当前年
// 				//部分数据为预估值,部分数据为实际值
// 				var solidLineData = [];	//实际值数据
// 				for(var i = 0 ; i < currMonth - 1; i++){
// 					solidLineData[i] = lastData[i];
// 				}
// 				index = 5;
// 				list = [firstData,secondData,thirdData,solidLineData,lastData];
// 			}else{
// 				//即全部数据都为实际值
// 				index = 0;
// 				list = [firstData,secondData,thirdData,lastData];
...
// 			//设置提示框
// 			/* var lineTooltip = textContent.getTooltip();
// 			lineTooltip.formatter = function(params,ticket,callback){
// 				var str = "";
// 				for (var i = 0; i < 4; i++) {
// 					if(params.length !== 4){
...
// 						}else{
// 							if(params[i].vaule !== undefined){
// 								str = str + params[i].seriesName + ":" + params[i].value + "<br/>";
// 							}else{
// 								str = str + params[i].seriesName + ":" + params[i+1].value + "<br/>";
// 							}
// 						}
// 					}else{
// 						str = str + params[i].seriesName + ":" + params[i].value + "<br/>";
...
// 		        return "load";
// 			};
// 			textContent.setContObject("tooltip",lineTooltip); */
// 			//设置网格
...
// 			var nextData = {"value": nextYear + "", "label": nextYear + ""};
// 			this.yearList.push(nextData);
// 			for(var i = 0; i < 1; i++){
// 				var data = {"value": (currYear - i) + "", "label":(currYear - i) + ""};
// 				this.yearList.push(data);
// 			}
// 			//1. 设置默认值			
	        
// 		},
// 		'params.scElectricityInfoDeatil.ym':function(){
// 			this.queryData();//查询列表信息
// 		}
	}
}); 
</script>
