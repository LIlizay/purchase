...
var meterVue = new Vue({
	...
	ready:function(){
		var me = this;
		me.queryparams.id = MainFrameUtil.getParams('metercountEdit').id;
		var methods={"confirmHandler":this.confirm};
		MainFrameUtil.setParams(methods,"metercountEdit");
		ComponentUtil.buildGrid("selling.deviationcheck.mpcount",$("#countGrid"),{
		    	...
		    	if(!(changeRows.addRows.length == 0 && changeRows.updateRows.length == 0 && changeRows.deleteRows.length == 0)){
		    		MainFrameUtil.confirm({
		    	        ...
		    	        onClose:function(type){
		    	            if(type=="ok"){//确定
		    	            	me.loadData(param.page,param.rows,true);
		    	            }else if(type=="cancel"){//取消
		    	            	me.loadData(me.pageNumber,param.rows,false);
		    	            }
		    	        }
		    	    });
		    		return false;
		    ...
		    columnsReplace:[
				{field:'meterReadPq',title:'抄表总示数',editor:{type:'numberbox',options:{
                    max:999999999.9999,
                    precision:4
                }}},
				{field:'meterReadDate',title:'抄表时间',editor:{type:'datetimebox',options:{required:true, editable:false,
				...
				{field:'usuallyDate',title:'是否抄表例日',editor:{type:'combobox',options:{
            		required:false,
           		 	method:"get",
                    valueField: 'value',
                    textField: 'name',
	...
	methods: {
		confirm: function(btn){
			...
			for(var a in rows){
				...
				if((rows[a].meterReadPq == null || rows[a].meterReadPq =="")&&(rows[a].usuallyDate ==null || rows[a].usuallyDate == "" ) ){
					$.messager.progress('close');
					$('#countGrid').datagrid('selectRow', a);
					MainFrameUtil.alert({title:"提示",body:"第"+ (parseInt(a) + 1) + "行抄表总示数和是否抄表例日需要填写至少一项！"});
					return;
				}
// 				if((rows[a].meterReadPq == null || rows[a].meterReadPq =="")&&(rows[a].usuallyDate == '0' ) ){
// 					$.messager.progress('close');
// 					$('#countGrid').datagrid('selectRow', a);
...
// 				}
				...
				//抄表例日重复验证
				if(rows[a].usuallyDate == "1"){
					if(typeof readMap[rows[a].meterReadDate.substring(0,7)] != "undefined"){
						$.messager.progress('close');
						$('#countGrid').datagrid('selectRow', a);
						flag = false;
						MainFrameUtil.alert({title:"提示",body:rows[a].meterReadDate.substring(0,7)+"月抄表例日有重复，请检查！"});
						return;
					}
			...
			if(changeRows.addRows.length == 0 && changeRows.updateRows.length == 0 && changeRows.deleteRows.length == 0){
           	...
           	if(flag){
              	//按钮不可点击
	           	...
	           	$.ajax({
					url:basePath+"scMpCountController/addScMpCountList",
					type:'post',
					data:$.toJSON(params),
		 			...
		 			success:function(data){
		 				...
		 				if(data.flag){
		 					if(data.extMap.flag){
		 						...
		 						me.oldList = $.parseJSON($.toJSON($('#countGrid').datagrid('getRows')));
		 					}else{
           		...
           		if(rows[i].id != undefined){
           			for(var j = 0 ; j < me.oldList.length ; j++){
           				if(rows[i].id == me.oldList[j].id){
           					if(rows[i].meterReadPq!=me.oldList[j].meterReadPq||rows[i].meterReadDate!=me.oldList[j].meterReadDate||rows[i].usuallyDate!=me.oldList[j].usuallyDate){
		...
		refreshAll: function(){
	    	...
	    	for(var i in rows){
	    		...
	    		var ed2 = $('#countGrid').datagrid('getEditor', {index:i,field:'usuallyDate'});
	    		rows[i].meterReadPq = $(ed.target).numberbox('getValue');
	    		rows[i].meterReadDate = $(ed1.target).datetimebox('getValue');
	    		rows[i].usuallyDate = $(ed2.target).combobox('getValue');
	    	}
		},
