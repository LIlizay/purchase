		...
		<mk-form-panel title="合同收益模拟" label_width="140px">
			<mk-form-row>
				...
				<mk-form-col :label="formFields.custName.label" :class="{'display-none':!formFields.custId.formHidden}" colspan="1" required_icon="true">
						<mk-selectbox :url="url" v-ref:consinfo :cacheurl="consCacheurl" height="620" width="50%"  textfield="consName" propname="id" :propvalue.sync="consId" name="consName" disabled="disabled"></mk-selectbox>
...
// 		  		discountLowerPlayFlag:true,//下限必填标志
			...
			ready:function(){
				...
				MainFrameUtil.setDialogButtons(me.getButtons(),"schemeEdit");
				me.initGrid();
				me.params.ssAgreScheme.id = MainFrameUtil.getParams("schemeEdit").id;
				me.getData();
			},
			methods: {
				initGrid:function(){
		            ...
		            $.ajax({
		                ...
		                success : function(data) {
		                    if(data.flag == true){
		                    	...
		                    	//me.consId = me.params.ssAgreScheme.consId;
		                    	me.consCacheurl = basePath + "cloud-purchase-web/sConsDialogController/getConsInfoById/"+data.data.consId;
		                    	if(me.params.ssAgreScheme.punishFlag == 1){
		        ...
		        getButtons:function(){
		        	var me = this;
		        	var buttons = [{text:"转成正式合同",type:"primary",handler:me.changeToContract},{text:"保存",type:"primary",handler:me.commit},{text:"取消",handler:function(){MainFrameUtil.closeDialog("schemeEdit")}}];
		            return buttons;
		        },
		      	//转换为合同
...
// 		        	me.contractParam.smDistMode.discountLowerAmt = me.params.ssAgreScheme.discountLowerAmt//优惠下限
						...
						success : function(result) {
							if(result.flag){
								 ...
								 MainFrameUtil.customAlert({
								        title:"提示",
								        body:"转换成功!",
								        buttons:[{text:'继续完善售电合同',type:"save",style:"default"},{text:'确定',type:"ok",style:"primary"}],
								        onClose:function(type){
								        	if(type=="save"){//继续完善售电合同
								    			var uuid=Math.random();
								        		MainFrameUtil.openTabPage(uuid, basePath+"view/cloud-purchase-web/agreement/smppa/list"
								        				, "售电合同管理",false);//打开售电合同管理页签
								        		MainFrameUtil.closeDialog("schemeEdit");//关闭编辑页面弹框
								            }
								        }
								    });
		    	            	...
		    	            	$.ajax({
		    	                    ...
		    	                    success : function(data) {
		    	                        if(data.flag == true){
		    	                             ...
		    	                             MainFrameUtil.closeDialog("schemeEdit");
		    	                        }else{
		    	                            MainFrameUtil.alert({
		    	                                title:"失败提示：",
		    	                                body:data.msg,
				...
				reset:function(){
					...
					me.params= {ssAgreScheme:{
							...
							settlementMode:"",
		    				diviCode:"",
		    				discountUpperFlag:null,
		    				discountUpperAmt:null,
		    				discountLowerFlag:null,
