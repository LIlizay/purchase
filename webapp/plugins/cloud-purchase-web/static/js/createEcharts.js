//存放option的变量
var contObject = {title:null,timeline:null,toolbox:null,tooltip:null,legend:null,grid:null,xAxis:[],yAxis:[],series:null};
var textContent = {
...
//			                }
                ...
                var seriesObj = this.getSeriesObj("angle",content.series,type);
                ...
                for(var j=0; j< content.legend.data.length; j++){
                    ...
                    contObj.type=type;
                    contObj.name = content.legend.data[j];
                    contObj.data = optionData[j];
                    seriesArray.push(contObj);
                }
                option.xAxis = xAxisObj;//设置xAxis
                option.yAxis = yAxisObj;//设置yAxis
                option.series = seriesArray;//设置series
                // 基于准备好的dom，初始化echarts实例
                var myChart = echarts.init(document.getElementById(dom));
                // 绘制图表
                myChart.setOption(option);
                ...
                var seriesObj = this.getSeriesObj("round",content.series,type);
                //生产series
                var seriesArray = new Array();
                for(var j=0; j< content.legend.data.length; j++){
                    seriesArray.push({value:optionData[j],name:content.legend.data[j]});
                }
                seriesObj.name = content.title.text;
                seriesObj.type = type;
                seriesObj.data = seriesArray;
                option.series = [seriesObj];//设置series
                // 基于准备好的dom，初始化echarts实例
                var myChart = echarts.init(document.getElementById(dom));
                // 绘制图表
                myChart.setOption(option);
    ...
    //清空contObject
    rest:function(){
        contObject = {title:null,timeline:null,toolbox:null,tooltip:null,legend:null,grid:null,xAxis:[],yAxis:[],series:null}
    },
    ...
    getSeries:function(){
        var seriesResult = {
            ...
            symbol:null,//标志图形类型，默认自动选择  (折线图、散点图)
