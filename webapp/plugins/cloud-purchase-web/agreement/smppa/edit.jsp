...
<div id="ppaEditVue" v-cloak>
	<su-form v-cloak v-ref:form1 id="fms1" offpos-style="true" :data-option="dataOption" :set-defaults="setDefaults" :data-validator.sync="valid" >
		<mk-panel title="用户信息" line="true" v-cloak>
			<mk-form-panel >
				<mk-form-row>
					<!-- 用户名称  -->
		            <mk-form-col :label="formFields.consName.label" required_icon="true" label-width="200px"
		                ...
		                <mk-selectbox disabled="disabled" :url="url" height="620" width="50%" textfield="consName" propname="id" 
		...
		<mk-panel title="合同基本信息" line="true" v-cloak>	
			<mk-form-panel>
				<mk-form-row>
					<!-- 合同编号  -->
		...
		<mk-panel title="双边电量分成方式" line="true" v-cloak>
		...
		<mk-panel title="用户偏差考核方式" line="true" v-cloak>
			<mk-form-panel>
				<mk-form-row>
					<!-- 生效条件  -->
		...
		<mk-panel title="售电公司违约惩罚方式" line="true" v-cloak>
			<mk-form-panel>
				<mk-form-row>
					<!-- 生效条件  -->
...
var ppaEditVue = new Vue({
    ...
    ready: function(){
    	...
    	$("#bidDivi0302").hide();
		//按钮初始化
		MainFrameUtil.setDialogButtons(this.getButtons(),"ppa-edit");
  		//初始化表单
    ...
    methods: {
		getButtons:function(){
			var buttons = [{text:"保存 ",type:"primary",handler:this.savePpaInfo},
			               {text:"取消 ",handler:function(){MainFrameUtil.closeDialog("ppa-edit");}}];
			return buttons;
		},
		//查询登录用户有无负荷预测权限
						...
						if(result.data){
							if(result.data.smPpa !== null){
								if(result.data.smPpa.distMode != "01"){
									that.setRow(result.data.smPpa,false);
								}
							}
							//双边分成方式
							that.params.smDistMode = result.data.smDistMode;
							that.params.smAgrePunishCons = result.data.smAgrePunishCons;
							that.params.smAgrePunishComp = result.data.smAgrePunishComp;
							if(result.data.smDistMode != null){
								//双边分成方式
								var diviCode = result.data.smDistMode.diviCode;
// 								if(diviCode == "03" || diviCode == "04" ||diviCode == "05"){
// 									that.partyBLcProp = result.data.smDistMode.partyBLcProp;
// 									that.partyALcProp = result.data.smDistMode.partyALcProp;
// 								}
								//竞价分成方式
								var bidDdiviCode = result.data.smDistMode.bidDiviCode;
								if(bidDdiviCode == "03" || bidDdiviCode == "04" ||bidDdiviCode == "05"){
									that.partyBBidProp = result.data.smDistMode.partyBBidProp;
									that.partyABidProp = result.data.smDistMode.partyABidProp;
								}
							}
							if(result.data.smPpa !== null && result.data.smPpa.consId !== null && result.data.smPpa.consId !== ""){
								that.urlCache = "${Config.baseURL}cloud-purchase-web/sConsDialogController/getConsInfoById/"+result.data.smPpa.consId;
							}
							//多附件
							if(result.data.smPpa !== null && result.data.smPpa.fileId !== null && result.data.smPpa.fileId !== ""){
								...
								try{
	          						if(eval('(' +result.data.smPpa.fileId + ')').fileName != null ||  eval('(' +result.data.smPpa.fileId + ')').fileName != ''){ //不是单附件的格式
		          						that.fileInfos.push({id :  eval('(' +result.data.smPpa.fileId + ')').fileId , name :  eval('(' +result.data.smPpa.fileId + ')').fileName});
	          						}	
          						}catch(err){ //单附件的格式
									...
									for(var i=0; i < attachments.length; i++){
										...
										if(fields.length == 2){
											that.fileInfos.push({id : fields[0] , name : fields[1]});
										}									
									}
          						}	
							}
							that.params.smPpa = result.data.smPpa;
						}
					}
				}
			});
		...
		setRow: function(content,flag){
			...
			this.initDataFlag = true; //初始化表格成功
			if((!flag && content.distMode == "02")|| (flag && this.params.smPpa.distMode == "02")){	//手动分解
				if(flag){
					this.initEditor(this.params.smPpa);
				}else{
					this.initEditor(content);
				}
			}
		},
		initEditor: function(smPpa){
			//月份差计算
    		var yearExpiry = smPpa.expiryDate.substr(0,4);
    		var monthExpiry = smPpa.expiryDate.substr(5,2);
    		var yearEffective = smPpa.effectiveDate.substr(0,4);
    		var	monthEffective = smPpa.effectiveDate.substr(5,2);
    		...
    		var index = smPpa.effectiveDate.substr(5,2) - 1;
			for(var i = 0 ; i < 12 ; i++){
				var mon = month[i];
				if(i>=index && i < index+num+1 ){
					$('#ppaEditGrid').datagrid('getColumnOption', mon).editor={id:mon,type:'numberbox',options:{precision:3,min:0,}};
				}else{
					$('#ppaEditGrid').datagrid('getColumnOption', mon).editor = undefined;
				}
			}
   			var rows = $("#ppaEditGrid").datagrid("getRows");
   			if(rows.length==0){
   				$('#ppaEditGrid').datagrid('appendRow',{});
   			}
   			$('#ppaEditGrid').datagrid('selectRow',0);
   			$('#ppaEditGrid').datagrid('beginEdit',0);
		},
		//上传回调
        uploadSuccess: function(data) {  //上传成功执行的方法
            ...
            for(var i = 0; i < fileList.length; i++){
          	...
          	//验证是否选择并上传了文件,如果是则设置文件id和文件名
          		ppaEditVue.params.smPpa.fileId = fileInfoStr;
	            this.save();
        },
    	...
    	save:function(){
			...
			if(me.params.smPpa.distMode == "02"){//手动分解
	    		...
	    		var eds = $('#ppaEditGrid').datagrid('getEditors', 0);
    		...
    		if(!me.getProxyPq()){
    		 ...
    		 if(str > me.params.smPpa.expiryDate){ 
              	me.params.smPpa.agreStatus = '03'; 					//当前日期 > 合同结束日期  已完成
              } else if(str < me.params.smPpa.effectiveDate){	//当前日期 < 合同开始日期 未开始
             	 me.params.smPpa.agreStatus = '01';
              } else {
             	 me.params.smPpa.agreStatus = '02';					//执行中
              }
    		//验证合同开始日期与结束日期不能跨年
    	...
    	distFunc:function(){
			...
			var month = ["jan","feb","mar","apr","may","jun","jul","aug","sept","oct","nov","dece" ];
    		//月份差计算
    		var yearExpiry = this.params.smPpa.expiryDate.substr(0,4);
    		var yearEffective = this.params.smPpa.effectiveDate.substr(0,4);
    	...
    	valueChange: function(){
   			...
   			if(this.params.smPpa.distMode == "01"){
       			...
       			this.distFunc();  
       		}else if(this.params.smPpa.distMode == "02"){	//手动分解
       			this.initEditor(this.params.smPpa);
       		}
    	},
    	//负荷预测
...
// 			    			url: "${Config.getConfig('selling.bigdata.url')}/powerFore/LongAssAction!predictLongAssDl.action?year=2018&consNo=4ddc5e74661f47ee89ecd40925bc0b97&av=b7b7596fffc91a2bc016c1128df857fd",
    ...
    watch:{
		...
		formFields:function(){
			this.initData(MainFrameUtil.getParams("ppa-edit").id);
