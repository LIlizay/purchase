			    	...
			    	<mk-form-row height="150px">
		             <!-- 意见回复 -->
		           	 	<mk-form-col :label="formFields.adminAnswer.label" colspan="3" :class="{'display-none':!formFields.adminAnswer.formHidden}">
			           		<su-textbox type="textarea" rows="7" rols="20" :data.sync="sSFeedback.adminAnswer"></su-textbox>
			            </mk-form-col>
		            </mk-form-row>
			    </mk-form-panel>
			</mk-panel>
	...
	<script type="text/javascript">
		...
		var listVue = new Vue({
			...
			ready : function(){
				...
				//按钮初始化
				MainFrameUtil.setDialogButtons(this.getButtons(),"reply");
				this.sSFeedback.id = MainFrameUtil.getParams("reply").id;
				this.initData();
				console.log(this.sSFeedback.id)
			},
			methods : {
				getButtons : function(){
					var buttons = [{text:"确认回复 ",type:"primary",handler:this.save},{text:"关闭 ",handler:function(){MainFrameUtil.closeDialog("reply");}}];
					return buttons;
				},
				initData : function(){
					var me = this;
