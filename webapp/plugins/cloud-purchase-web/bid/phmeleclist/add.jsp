	...
	<mk-form-panel slot="top" title="电费清单新增" num="2">
		<mk-form-row>
	        ...
	        <mk-form-col :label="formFields.ym.label" required_icon="true">
	        	<su-datepicker :data.sync="params.ym" format="YYYYMM" ></su-datepicker>
	        </mk-form-col>
    	</mk-form-row>
    	<mk-form-row>
    		 <!-- 营销户号 -->
					    ...
					    options:{
					        ...
					        onChange:function(value,oldVal){
					        	if(value == "01"){
					        		...
					        		var editor = $("#addGrid").datagrid("getEditor",{index:rowIndex,field:'meterNo'});
	...
	methods: {
		save:function(backFun){
			var me = this;
	  		...
	  		var total =  $('#addGrid').datagrid("getRows").length;
		...
		del:function(){
			...
			MainFrameUtil.confirm({
      	        ...
      	        onClose:function(type){
      	        	if(type=='ok'){
      	        		for(var i in rows){
      	        			var index = $('#addGrid').datagrid("getRowIndex",rows[i]);
		...
		add:function(){
			 $('#addGrid').datagrid("insertRow",{index:0,row:{}});
