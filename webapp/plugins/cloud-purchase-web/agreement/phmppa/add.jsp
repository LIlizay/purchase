				...
				<mk-form-row>
					<!-- 合同类型-->
			    	 <mk-form-col :label="formFields.agreType.label" :class="{'display-none':!formFields.agreType.formHidden}">
		            	<su-select url="${Config.baseURL}globalCache/queryCodeByParam?domain=selling&pCode.codeType=purchase_agreType&valueIn" 
...
<%-- 		    			<su-select url="${Config.baseURL}globalCache/queryCodeByKey/pcode/selling/sell_punishFlag" label-name="name" name="lowerCheckFlag" --%>
<!-- 		    			   :select-value.sync="params.phmAgreDeviation.lowerCheckFlag" @su-change="lowerCheckFlagChange"></su-select> -->
<!-- 					</mk-form-col> -->
<!-- 				</mk-form-row> -->
...
var ppaAddVue=new Vue({
    ...
    data:{
        ...
        params:{
        	...
        	phmAgreDeviation:{
        		upperCheckFlag:null,//售电公司正偏差
        		upperPqProp:null,//允许正偏差范围
        		upperCheckPrcType:"",//惩罚基准值
        		upperCheckPrcProp:null,//惩罚比例
        		upperCheckPrc:null,//正偏差考核电价
        		lowerCheckFlag:null,//售电公司负偏差
        		lowerPqProp:null,//允许负偏差范围
        		lowerCheckPrcType:"",//惩罚基准值
        		lowerCheckPrcProp:null,//惩罚比例
        		lowerCheckPrc:null,//负偏差考核电价
        	},
    ...
    ready:function(){
		...
		ComponentUtil.getFormFields("purchase.agreement.phmppa",this);
  	  	MainFrameUtil.setDialogButtons([{text:"保存",type:"primary",handler:this.save},{text:"取消",handler:function(){MainFrameUtil.closeDialog("ppaAdd")}}],"ppaAdd");
  	  	//初始化列表
  	  	this.initGrid();
	},
    methods : {
    	...
    	upperTypeChange:function(){
    		if(this.params.phmAgreDeviation.upperCheckPrcType == "01"){//人工录入
    			...
    			this.params.phmAgreDeviation.upperCheckPrc = null;
    		}
    	},
    	...
    	edit:function(){
    		...
    		var bgnMonth = parseInt(this.params.phmPpa.bgnDate.split("-")[1]);
    		var endMonth = parseInt(this.params.phmPpa.endDate.split("-")[1]);
    		MainFrameUtil.openDialog({
	  			id:"generatorAdd",
	  			params:{row:newRows,elecId:elecId, bgnMonth: bgnMonth, endMonth:endMonth},
				href:'${Config.baseURL}view/cloud-purchase-web/agreement/phmppa/generator-add',
				iframe:true,
				modal:true,
				width:"80%",
		...
		getPqRows:function(){
			...
			for(var i=0;i<rows.length;i=i+2){
				...
				var phmGenePqDist={generatorId:qpRow.generatorId,totaPq:qpRow.totaPq,distributionMode:qpRow.distributionMode}
        		...
        		success:function(data){
        			if(data.flag){
        				if(data.msg == "在该有效期内已存在该电厂合同！"){
      					...
      					MainFrameUtil.alert({title:"提示",body:data.msg,onClose:function(){MainFrameUtil.closeDialog("ppaAdd");}});
      				}else{
        ...
        save : function() {
        	...
        	if(rows.length > 0){
	        	...
	        	if(totalPq != this.params.phmPpa.agrePq){
	        		MainFrameUtil.alert({title:"提示",body:"机组总电量与合同电量不相等！"});
					return;
	        	}
        	}
        	 //验证是否选择文件
				...
				buttons:[{
                    ...
                    handler:function(btn,params){
                    	params.save(function(row){
                    		...
                    		me.elecTypeCode = row.elecTypeCode;
	...
	watch:{
		'params.phmPpa.bgnDate':function(newValue,oldValue){
			//合同的开始与结束日期不允许跨年
			if(this.params.phmPpa.bgnDate != '' && this.params.phmPpa.endDate != ''){
				if((this.params.phmPpa.bgnDate).substring(0,5) != (this.params.phmPpa.endDate).substring(0,5)){
					MainFrameUtil.alert({title:"提示",body:"合同开始日期与合同结束日期不允许跨年！"});
				}
			}
