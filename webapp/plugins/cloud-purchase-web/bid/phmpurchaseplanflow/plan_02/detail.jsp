...
<html>
<head>
<title>购售电交易-月度购电管理竞价交易申报添加交易申报详情</title>
...
<body id="examineVue" style="height:100%" v-cloak>
	<div id="transaction" style="height:90%">
		...
		<mk-tabpanel height="80%"  auto_fit="true" :selected.sync="selectedTabIndex">
	         <mk-tabpage index=0 height="100%" header="售电公司申报明细" :href.sync="transactionReportDetailPage" iframe="false" selected="true"></mk-tabpage>
	         <!-- 加载远程代码片段   iframe=false -->
	         <mk-tabpage index=1 height="100%" header="委托电量明细" :href.sync="proxyPqDetailPage" iframe="false" ></mk-tabpage>
	     </mk-tabpanel>
	     <br/>
     </div>
...
var detailVue = new Vue({
	...
	data:{
		...
		proxyPqDetailPage:"${Config.baseURL}view/cloud-purchase-web/bid/phmpurchaseplanflow/plan_02/proxyPqDetailD",//href的绑定属性
		formFields:{},
	...
	ready:function(){
		...
		if(id.substr(id.length-2) == "sl"){
			...
			me.initData();
			me.transactionReportDetailPage = "${Config.baseURL}view/cloud-purchase-web/bid/phmpurchaseplanflow/plan_02/transactionReportDetailD_sl";
		}else{
			me.sum.id = id;
			me.initData();
			me.transactionReportDetailPage = "${Config.baseURL}view/cloud-purchase-web/bid/phmpurchaseplanflow/plan_02/transactionReportDetailD";
// 			me.transactionReportDetailPage = "${Config.baseURL}view/cloud-purchase-web/bid/phmpurchaseplanflow/plan_02/transactionReportDetailD_sl";
		}
