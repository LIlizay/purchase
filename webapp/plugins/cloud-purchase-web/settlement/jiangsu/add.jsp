...
var settlementListVue=new Vue({
    ...
    ready:function(){
    	var that = this;
    	that.formData =MainFrameUtil.getParams("sellSettlementAdd").formData;
    	that.saveData.settleId = that.formData.id;
    	that.saveData.ym = that.formData.ym;
    	//设置按钮组
    	MainFrameUtil.setDialogButtons(this.getButtons(),"sellSettlementAdd");
    	//获取选择分成模式下拉框的内容
   		$.ajax({
    	...
    	//用户月度电量分配列表
       	$("#userPqGrid").datagrid({
       		url: basePath+"cloud-purchase-web/smPurchaseSchemeDetailController/getConsInfoByYmOrSchemeId?ym=" + that.formData.ym,
       		method: 'get',
            loadMsg:"请稍后",
            width:'100%',
            height:"100%",
            ...
            columns:[
				...
				{field:'agrePrc',title:'保底协议价 </br>（元/兆瓦时）',width:100,align:'center',
				...
				{field:'diviValue',title:'分成基准值 </br>（元/兆瓦时）',width:100,align:'center',
				...
				{field:'partyALcProp',title:'用户双边</br>分成比例 （%）',width:100,align:'center',
				...
				{field:'bidDiviCode',title:'分成模式',width:100,align:'center',
					editor:{type:'combobox',options:{
                        valueField: 'value',
                        textField: 'name',
                        data: that.divideWayCodeList
				...
				{field:'bidAgrePrc',title:'保底协议价 </br>（元/兆瓦时）',width:100,align:'center',
				...
				{field:'partyABidProp',title:'用户竞价</br>分成比例 （%）',width:100,align:'center',
				...
				{field:'actualPq',title:'实际用电量 </br>（兆瓦时）',width:100,align:'center',
				...
				{field:'deliveryPrc',title:'合同加权平均价</br>（元/兆瓦时）',width:100,align:'center',
				...
				{field:'serviceAmt',title:'服务费用</br>（元）',width:100,align:'center',
					editor:{type:'numberbox',options:{precision:4}},
					formatter:function(value,index,row){
						  if(value){
							  return parseFloat(value);
    	...
    	//用户偏差费用计算表
       	$("#deviationAmtGrid").datagrid({
       		url: basePath+"cloud-purchase-web/smConsDeviationAmtController/getConsDeviAmtByYmOrSchemeId?ym=" + that.formData.ym,
       		method: 'get',
            loadMsg:"请稍后",
            width:'100%',
            height:"100%",
            ...
            columns:[[
				...
				{field:'devPqProp',title:'偏差电量比例 </br>（%）',width:'8%',align:'center'},
				{field:'punishType',title:'偏差是否考核',width:'8%',align:'center',
					editor:{type:'combobox',options:{
                        valueField: 'value',
                        textField: 'name',
				...
				{field:'upperThreshold1',title:'多用1段考核比例</br>（%）',width:'10%',align:'center',
				...
				{field:'upperDevAmt1',title:'多用1段考核金额</br>（元）',width:'10%',align:'center',
				...
				{field:'upperThreshold2',title:'多用2段考核比例</br>（%）',width:'10%',align:'center',
				...
				{field:'upperDevAmt2',title:'多用2段考核金额</br>（元）',width:'10%',align:'center',
...
//        	   	}
        ...
        getButtons:function(){
            var buttons = [{text:"保存",type:"primary",handler:this.saveValidate},
                           {text:"取消",handler:function(){MainFrameUtil.closeDialog("sellSettlementAdd")}}];
            return buttons;
        },
...
//     	   }
        	...
        	$.messager.progress();
			//保存和关闭按钮失效
           	$("#sellSettlementAdd-window div.dialog-buttons button" , parent.document).attr("disabled","disabled");
    	   $.ajax({
				...
				data:$.toJSON(saveData),
	 			...
	 			success:function(data){
                	...
                	//保存和关闭按钮生效
    	           	$("#sellSettlementAdd-window div.dialog-buttons button" , parent.document).attr("disabled",false);
	 				if(data.flag){
                		MainFrameUtil.alert({title:"成功提示",body:data.msg});
