...
// 				title:'用户用电量收集记录新增',
		...
		edit : function(){
			...
			$.ajax({
				url : basePath+'smConsPowerViewController/getCountIds?consId=' + rows[0].consId,
				type : "get",
				success : function(data){
					if(data.flag != "true"){
						MainFrameUtil.alert({ title : "提示", body : "请先维护用户的表计示数！" }); 
					}else{
						MainFrameUtil.openDialog({
				  			id:"conspowerAdd",
				  			params:{"row":rows[0]},
		...
		detail : function(index){
			...
			$.ajax({
				url : basePath+'smConsPowerViewController/getCountIds?consId=' + row.consId,
				type : "get",
				success : function(data){
					if(data.flag != "true"){
						MainFrameUtil.alert({ title : "提示", body : "请先维护用户的表计示数！" }); 
