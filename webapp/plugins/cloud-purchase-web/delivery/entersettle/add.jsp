	    	...
	    	<mk-form-panel label_width="180px" num="2">
	    		<mk-form-row >
		    		...
		    		<mk-form-col :label="formFields.ym.label" required_icon="true">
		    			<su-datepicker format="YYYY-MM" :data.sync="params.phmEnterSettle.ym" name="ym"></su-datepicker>
		    		</mk-form-col>
		    		<!-- 售电公司购电收益 -->
	...
	<script type="text/javascript">
		...
		var ruleAddVue =new Vue({
			...
			data: {	
				...
				consCheck:null,
			    params:{
			    	phmEnterSettle:{ym:null,comProfit:null,comCheck:null},
			...
			ready:function(){
				...
				MainFrameUtil.setDialogButtons(me.getButtons(),"settle_add");
				me.initGrid();
			},
			methods: {
				getMonth:function(){
					...
					me.params.phmEnterSettle.ym = date.getFullYear()+"-"+month;
					me.changeCount += 1;
					$.ajax({
						url: basePath + 'cloud-purchase-web/enterSettleController/checkRepeatByMonth',
						type: 'post',
						...
						success: function(data) {
							if(data.flag){
								if(!data.data){
									me.params.phmEnterSettle.ym = null;
								}
							}else{
		 		...
		 		initGrid:function(){
		 			...
		 			ComponentUtil.buildGrid("purchase.delivery.enterdetail",$("#settleGrid"),{ 
						url: basePath + 'cloud-purchase-web/enterSettleController/getConsDetailByYm',
					    width:"100%",
					    height:"270px",
					    method: 'post',
					    singleSelect:true,
					    ...
					    onLoadSuccess: function(data){
					    	...
					    	var actPqTotal = 0.000;
							var checkPqTotal = 0.000;
					    	for(var i = 0 ; i < rows.length ; i++){
					    		$("#settleGrid").datagrid('beginEdit', i);
					    		reportPqTotal += rows[i].reportPq;
					    		if(rows[i].actPq != null){
					    			actPqTotal =  actPqTotal + parseFloat(rows[i].actPq);
					    			var checkPq = parseFloat(rows[i].actPq) - parseFloat(rows[i].reportPq);
					    			$("#settleGrid").datagrid("getPanel").find(".datagrid-row [field='consCheckPq']").eq(i).children().text(parseFloat(checkPq).toFixed(3));
					    			checkPqTotal = checkPqTotal + parseFloat(checkPq);
					    		}
					    	}
					    	me.actPq = parseFloat(actPqTotal).toFixed(3);
							me.checkPq = parseFloat(checkPqTotal).toFixed(3);
		  	    ...
		  	    save:function(){
		  	    	...
		  	    	$.ajax({
						...
						success: function(data) {
							if(data.flag){
								MainFrameUtil.closeDialog("settle_add");
							}else{
								MainFrameUtil.alert({title:"提示",body:"请刷新页面重试！"});
							}
						},
		        ...
		        getButtons:function(){
		        	var me = this;
		        	var buttons = [{text:"保存",type:"primary",handler:me.save},{text:"取消",handler:function(){MainFrameUtil.closeDialog("settle_add")}}];
		            return buttons;
		        }
			},
			watch:{
				"params.phmEnterSettle.ym" : function(value){
					var me = this;
					if(me.changeCount > 1){
						$.ajax({
							url: basePath + 'cloud-purchase-web/enterSettleController/checkRepeatByMonth',
							...
							success: function(data) {
								if(data.flag){
									if(data.data){
										...
										MainFrameUtil.alert({title:"提示",body:"该购电年月已结算！"});
										me.params.phmEnterSettle.ym = null;
									}
								}else{
									MainFrameUtil.alert({title:"提示",body:"请刷新页面重试！"});
